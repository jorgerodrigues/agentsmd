...
Each round is a fresh session with no memory of the last one, so a decision you make about a finding does not reach the reviewer unless you carry it. A finding you decline goes back with its reason on the next round; without that, the same objection returns every round and each return costs a full review.
...
`review-pr` knows how to read that block; the notice text is what tells it these are answered rather than unseen.
...
From round two, match every finding against `$DIR/declined.md` before you read any code. `review-pr` marks a finding it brings back as `(re-raised)`, but decide from the content, not the tag. Two outcomes:
...
- **Rebutted** — the reviewer names what is wrong with your reason and cites code. That is a live candidate: hold it to the bar and check the claim against the code as if it were new. A rebuttal is the mechanism working, not the reviewer being stubborn.
...
Findings you declined do not block the exit, and after round one they should be rare — the reviewer has been told about them. A restatement is dropped per step 3 and counts for nothing here: not toward the exit, not toward circling. A **rebuttal** is a live finding and is triaged like any other; whatever you then do with it is an ordinary ledger row.

### Keep a ledger

From round two onward, maintain a ledger with one row per finding: the round it appeared, its fingerprint, **the underlying claim restated in your own words**, what you did about it, and — for anything you declined — **the reason you handed back**. A later rebuttal can only be judged against what the reviewer was actually told.
...
- A claim you declined, then re-declined after a rebuttal, comes back rebutted a third time. You are negotiating, not converging. Stop and show the user the claim, your reason, and the reviewer's rebuttal.
